...
const MonotributistaInput = ({
    ...
    const createMonotribuType = async () => {
        const response = await axiosPost(API_ROUTES.monotributoTypeDir.monotributoType, {
    ...
    const getMonotributoTypeList = async () => {
        const response = await axiosGetQuery(API_ROUTES.monotributoTypeDir.monotributoType, [])
        if (!response.error) {
    ...
    return (<>
            ...
            <FormGroup>
                ...
                <InputGroup>
                    ...
                    <Input
                        ...
                        onChange={e => setMonotributoType({ ...monotributoType, monotributoTypeId: e.target.value })}
                    >
                        <option value={false}>Seleccione tipo monotributo...</option>
                        {
                            monotributoList.length > 0 && monotributoList.map((team, index) => {
                                return (
                                    <option
                                        key={index}
                                        value={team.id}
                    ...
                    <InputGroupAddon addonType="append">
                        ...
                        <Tooltip placement="top" isOpen={tooltipOpen} target="addMonoBtn" toggle={() => setTooltipOpen(!tooltipOpen)}>
        ...
        <Modal isOpen={modalOpen} toggle={() => setModalOpen(!modalOpen)}>
            ...
            <ModalBody>
                ...
                <FormGroup>
                    <Label for="teamDescription">Descripción del tipo de monotributo</Label>
                    <Input
                        type="textarea"
                        id="teamDescription"
