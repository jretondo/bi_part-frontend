...
const GrossIncomeInput = ({
    ...
    const getGrossIncomeList = async () => {
        const response = await axiosGetQuery(API_ROUTES.grossIncomesDir.grossIncomes, [])
        if (!response.error) {
                ...
                <FormGroup>
                    ...
                    <InputGroup>
                        ...
                        <Input
                            ...
                            onChange={e => setGrossIncome({ ...grossIncome, grossIncomeId: e.target.value })}
                        >
                            <option value={false}>Seleccione un impuesto...</option>
                            {
                                grossIncomeList.length > 0 && grossIncomeList.map((grossIncome, index) => {
                                    return (
                                        <option
                                            key={index}
                                            value={grossIncome.id}
                                        >
                                            {grossIncome.name}
                                        </option>
                                    )
                        ...
                        <InputGroupAddon addonType="append">
                            ...
                            <Tooltip placement="top" isOpen={tooltipOpen} target="addIIBBBtn" toggle={() => setTooltipOpen(!tooltipOpen)}>
                                Agregar más IIBB
                            </Tooltip>
                        </InputGroupAddon>
                    </InputGroup>
